...
export default async function handler(req, res) {
  ...
  try {
    // Call Anthropic API using your secret key (stored in Vercel environment)
    const response = await fetch('https://api.anthropic.com/v1/messages', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'x-api-key': process.env.ANTHROPIC_API_KEY, // Secret — users never see this
        'anthropic-version': '2023-06-01'
      },
      body: JSON.stringify({
        model: 'claude-sonnet-4-20250514',
        max_tokens: 1000,
        system: `You are Buddy — a sharp, friendly AI companion. You talk like a smart, caring friend — warm but direct.
